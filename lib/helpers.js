...
var getInstalledModules = function getInstalledModules() {
  ...
  for (
    var _i = 0, _Object$keys = Object.keys(dependencies);
  ...
  for (
    var _i2 = 0, _Object$keys2 = Object.keys(devDependencies);
...
var installModule = function installModule(_ref, notifyMode) {
  ...
  var spinner = startSpinner(
    "".concat(colors.green("Installing"), " ").concat(name, "\n"),
    "green"
  );
  ...
  if (success) stopSpinner(spinner, message, "green", notifyMode);
    ...
    stopSpinner(
      spinner,
      "".concat(name, ' "installation failed"'),
      "yellow",
      notifyMode
    );
};
...
var installModulesandScopedModules = function installModulesandScopedModules(
  _ref2,
  notifyMode
) {
  var name = _ref2.name,
...
var getUninstallCommand = function getUninstallCommand(name) {
  ...
  var command;
  if (packageManager === "npm")
    command = "npm uninstall ".concat(name, " --save");
  else if (packageManager === "yarn") command = "yarn remove ".concat(name);
  return command;
};
/* Uninstall module */
...
var uninstallModule = function uninstallModule(_ref3, notifyMode) {
  ...
  var spinner = startSpinner(
    "".concat(colors.red("Uninstalling"), " ").concat(name),
    "red"
  );
  runCommand(command);
