...
var detective = function detective(src, options) {
      ...
      while (1) {
        switch (_context3.prev = _context3.next) {
          ...
          case 2:
            ...
            try {
              cp = execa.sync(cmd, args); //conditional so Cleanup funciton doesn't display any messages to console

              if (notifyMode) showNotification(message);
              if (moduleName) console.log(colors.green("Added ==>"), cp.stdout.split("\n")[0]);
            } catch (err) {
