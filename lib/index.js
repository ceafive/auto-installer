...
var main = function main() {
  ...
  if (uninstallMode) {
    ...
    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        ...
        helpers.uninstallModule(_module, notifyMode);
      }
    } catch (err) {
      _iterator.e(err);
    } finally {
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      helpers.installModulesandScopedModules(_module2, notifyMode);
    }
  } catch (err) {
    _iterator2.e(err);
  } finally {
  ...
  helpers.cleanup(); // if (!watchersInitialized) initializeWatchers();
};
